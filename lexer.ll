...
print					{ log_lexer(yytext); return yy::parser::make_PRINT(yytext); }
 ...
 /* Values */
false					{ log_lexer(yytext); return yy::parser::make_VALUE(yytext); }
true					{ log_lexer(yytext); return yy::parser::make_VALUE(yytext); }
[0-9]+\.[0-9]+	 		{ log_lexer(yytext); return yy::parser::make_VALUE(yytext); }
[0-9]+					{ log_lexer(yytext); return yy::parser::make_VALUE(yytext); }

\"[^\*\"]*\"			{ log_lexer(yytext); std::string str = yytext; return yy::parser::make_VALUE(str.substr(1, yyleng - 2)); }
